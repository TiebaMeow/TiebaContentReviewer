...
- **水平扩展**：基于 Redis Consumer Group 机制，支持多 Worker 实例水平扩展以提升吞吐量。
...
规则以 JSON 格式存储，支持复杂的逻辑嵌套。例如，定义一个规则：当帖子内容包含 "违规词" 且 (用户等级 < 3 或 发帖量 < 5) 时，执行 "删除" 和 "封禁" 操作。
